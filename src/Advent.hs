...
charGrid :: Text -> (HashMap (Complex Int) Char, Map Char (HashSet (Complex Int)))
charGrid t =
  let xs = do
        (i, l) <- zip [0, -1 ..] $ lines t
...
findGridChar :: Complex Int -> Map Char (HashSet (Complex Int)) -> Maybe Char
findGridChar z =
  fmap fst
    . find (HS.member z . snd)
    . M.toList

gridRange :: Map Char (HashSet (Complex Int)) -> (Int, Int, Int, Int)
gridRange g =
...
printGrid :: Map Char (HashSet (Complex Int)) -> IO ()
printGrid g = do
...
instance {-# OVERLAPPING #-} Num (Complex Int) where
  (x :+ y) + (x' :+ y') = (x + x') :+ (y + y')
  (x :+ y) - (x' :+ y') = (x - x') :+ (y - y')
